...
public:
    ...
    Window(VSGViewer *win, vsg::ref_ptr<vsg::WindowTraits> traits)
        : Inherit(traits)
        , _window(win)
    {
...
struct VSGViewer::Private
{
    ...
    vsg::ref_ptr<vsg::Group> layoutsRoot{vsg::Group::create()};
    vsg::ref_ptr<vsg::Node> skybox{vsg::Node::create()};
...
/*
QColor VSGViewer::clearColor() const
{
    if (p->window.valid())
    {
...
*/
VSGViewer::~VSGViewer() = default;

void VSGViewer::exposeEvent(QExposeEvent *e)
{
    if (isExposed())
    {
        if (!p->initialized)
        {
            ...
            p->options = vsg::Options::create();
            p->options->fileCache = vsg::getEnv("RRS_FILE_CACHE");
            p->options->paths = vsg::getEnvPaths("RRS_EDITOR_FILE_PATH");
            p->options->add(vsgXchange::all::create());
...
#if 1
                vsg::Path osm = vsg::findFile("/home/asafr/Documents/dems/out/out.osgt", p->options);

                auto globe = vsg::read(osm, p->options);
                if (auto node = globe.cast<vsg::Node>(); node)
                {
                    p->scenegraph->addChild(node);
                }


 //               p->scenegraph = p->modelRoot->getChild(0);
#endif
              ...
              //  auto lookAt = vsg::LookAt::create(centre+vsg::dvec3(0.0, -radius*3.5, 0.0), centre, vsg::dvec3(0.0, 0.0, 1.0));
                vsg::ref_ptr<vsg::EllipsoidModel> ellipsoidModel(p->scenegraph->getObject<vsg::EllipsoidModel>("EllipsoidModel"));
                ...
                if (ellipsoidModel)
                {
                    ...
                    lookAt->eye = ellipsoidModel->convertLatLongAltitudeToECEF(vsg::dvec3(44.3833,40.6499, 10000.0));
                    lookAt->center = ellipsoidModel->convertLatLongAltitudeToECEF(vsg::dvec3(44.3833,40.6499, 0.0));
                    lookAt->up = normalize(vsg::cross(lookAt->center, vsg::dvec3(-lookAt->center.y, lookAt->center.x, 0.0)));
                ...
                p->scenegraph->addChild(p->skybox);
                p->scenegraph->addChild(p->layoutsRoot);
                ...
                p->viewer->addEventHandler(vsg::Trackball::create(p->camera, ellipsoidModel));
...
void VSGViewer::keyPressEvent(QKeyEvent *e)
{
...
void VSGViewer::keyReleaseEvent(QKeyEvent *e)
{
    vsg::KeySymbol keySymbol, modifiedKeySymbol;
    vsg::KeyModifier keyModifier;
...
void VSGViewer::mouseMoveEvent(QMouseEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
void VSGViewer::mousePressEvent(QMouseEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
void VSGViewer::mouseReleaseEvent(QMouseEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
void VSGViewer::resizeEvent(QResizeEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
void VSGViewer::moveEvent(QMoveEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
void VSGViewer::wheelEvent(QWheelEvent *e)
{
    vsg::clock::time_point event_time = vsg::clock::now();
...
/*
void VSGViewer::setClearColor(const QColor &color)
{
    VkClearColorValue clearColor = {
        {
            (float)color.redF(),
...
*/
bool VSGViewer::loadFile(const QString &filename, vsg::ref_ptr<vsg::Group> parent)
{
     qCDebug(lc) << "Load node " << filename;
     vsg::Path path = vsg::findFile(filename.toStdString(), p->options);
     auto file = vsg::read(path, p->options);
     if (auto node = file.cast<vsg::Node>(); node)
     {
...
SceneModel* VSGViewer::getSceneModel()
{
    return new SceneModel(p->scenegraph);
}

void VSGViewer::loadToRoot(const QString &filename)
{
     loadFile(filename, p->scenegraph);
}

vsg::Instance *VSGViewer::instance()
{
    return p->vsgInstance;
}

void VSGViewer::render()
{
