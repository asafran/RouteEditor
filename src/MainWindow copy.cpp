...
QWindow* MainWindow::initilizeVSGwindow()
{
    ...
    horizonMountainHeight = settings.value("HMH", 0.0).toDouble();
    ...
    viewerWindow->initializeCallback = [&](vsgQt::ViewerWindow& vw) {
        ...
        // set up the camera
        vsg::ref_ptr<vsg::Camera> camera(updateLookAt(window));
...
vsg::Camera *MainWindow::updateLookAt(const vsg::Window *window)
{
    ...
    return vsg::Camera::create(perspective, lookAt, vsg::ViewportState::create(window->extent2D())).release();
}
...
void MainWindow::loadToRoot(QString &filename)
{
    if (auto node = vsg::read_cast<vsg::Node>(filename.toStdString()); node.valid())
    {
        ...
        auto& window = viewerWindow->proxyWindow;
        if (!window) return;

        auto& viewer = viewerWindow->viewer;
        if (!viewer) viewer = vsg::Viewer::create();
